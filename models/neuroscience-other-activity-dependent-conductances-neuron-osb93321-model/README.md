# 93321

**Source**: [opensourcebrain](https://github.com/OpenSourceBrain/93321)
**Standard**: other
**Authors**: Open Source Brain

## Description

Activity dependent conductances in a neuron model  (Liu et al. 1998)


## Usage

This model was auto-generated from the opensourcebrain repository.

```yaml
# In a space.yaml wiring file:
models:
  - repo: Biosimulant/models
    alias: model
    manifest_path: models/neuroscience-other-activity-dependent-conductances-neuron-osb93321-model/model.yaml
```

## Tags

other, opensourcebrain, auto-generated, neuroscience
